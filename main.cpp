...
/**
 ...
 THE CODE I HAVE WRITTEN IS EXTENSIBLE FOR MULTIPLE PARKING LOTS WITH MULTIPLE SPACES, BUT FOR NOW I HAVE IMPLEMENTED FOR ONLY ONE PARKING LOT AS BASED ON THE INPUT.TXT
...
class parkingLot{ // parking lot class
    private:
        ...
        map<int,vehicle> slot_details ; // map of slots having details of vehicle parked
        map<string,int> where_ ; // map storing the slot number where a vehicle is parked
        string current_status(){ // returning a status string that can be parsed and used for various uses
            if( MAX_SIZE == -1 ){
                return "INVALID" ;
            }
            if( MAX_SIZE == load ){
    ...
    public:
            ...
            int whereIsVehicleParked(string regNo){
                return where_[regNo] ;
            }
            int park(int age, string regno){
                int free_slot = (*inActive_slots.begin()) ; // get the first slot that is free
...
class globalHandler{ // global class that handles multiple parking lots with spaces
    public:
    ...
    void create_parking_lot(int n){ // create a new parking lot
        parkinglots++ ;
        curr_parking_lot = parkinglots ; // based on the input arguments always the current parking lot is to be reffered
    ...
    string make_To_String(vector<string> v){ // helper function to combine strings
        string str="" ;
        for(auto& s: v){
            str+= s + " " ;
        }
    ...
    void park(int age, string regNo){
        if( allVehicles.count(regNo) ){ // if the vehicle is already parked no need to park it again
            auto details = allVehicles[regNo].getDetails() ;
            print("Sorry car with registration number already parked in parking lot "<<allVehicles[regNo].get_parkedIn()<<" in slot number "<<parkingLots[curr_parking_lot].whereIsVehicleParked(regNo)) ;
            return ;
        }
        if( curr_parking_lot == -1 ){// if no parking lot is created, have to create a parking lot
        ...
        if(status == "INVALID"){
            ...
            print(make_To_String({ "Car with vehicle registration number", ("\"" + regNo + "\"") , "has been parked at slot number " + to_string(parkedin)  }))  ;
    ...
    void get_slot_numbers_of_age(int age){
        vector<int> v  ;
        for(auto& e: allVehicles){ // get all the vehicles slot where driver age  == given age
        ...
        sort(begin(v),end(v)) ;
        if(v.size() == 0 ){
            print("Sorry no vehicle parked with the given age")
            ret
        }
        for(auto& e: v){
            cout<<e<<" " ;
    ...
    void get_slot_numbers_of_regNo(string regNo){
        vector<int> v  ;
        for(auto& e: allVehicles){// get all the vehicles slot where registration No.  == given registration No.
        ...
        sort(begin(v),end(v)) ;
        if(v.size() == 0 ){
            print("Sorry no vehicle parked with the given registration number")
            ret
        }
        for(auto& e: v){
            cout<<e<<" " ;
    ...
    void vacate(int n){
        ...
        if(p.first){
            ...
            print(make_To_String({ "Slot no", to_string(n), "vacated, the car with vehicle registration no", "\"" + details.second + "\"" , "left the space, the driver of the car was of age", to_string(details.first) })) ;
    ...
    void get_registration_numbers_of_age(int age){
        vector<string> v  ;
        for(auto& e: allVehicles){// get all the vehicles registration No. where driver age  == given age
            if(e.second.getAge() == age ){
        ...
        sort(begin(v),end(v)) ;
        if(v.size() == 0 ){
            print("Sorry no vehicle found with the given age")
            ret
        }
        for(auto& e: v){
            cout<<e<<" " ;
